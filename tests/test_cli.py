...
def test_cli_no_args_shows_startup_screen(capsys):
    ...
    assert "COMMANDS & USAGE:" in captured.out
...
def test_cli_analyze_terminal_output(tmp_path: Path, capsys):
    img_path = tmp_path / "cli_test.jpg"
    img = Image.new("RGB", (100, 100), color=(10, 20, 30))
    img.save(img_path, format="JPEG")

    ret = main(["analyze", str(img_path)])
    assert ret == 0
    captured = capsys.readouterr()
    assert "IMAGE INTELLIGENCE & FORENSICS" in captured.out
    assert "cli_test.jpg" in captured.out
    assert "Dimensions  : 100 × 100 px" in captured.out
    assert "Analysis complete." in captured.out


def test_cli_analyze_json_output(tmp_path: Path, capsys):
    img_path = tmp_path / "cli_json_test.png"
    img = Image.new("RGB", (150, 75), color=(50, 50, 50))
    img.save(img_path, format="PNG")

    ret = main(["analyze", str(img_path), "--json"])
    assert ret == 0
    captured = capsys.readouterr()
...
def test_cli_non_existent_file(capsys):
    ret = main(["analyze", "does_not_exist_file_99.jpg"])
    assert ret == 1
    captured = capsys.readouterr()
