...
from eagle.schemas import LocationInfo
...
def _parse_location_payload(payload: dict) -> LocationInfo:
...
async def reverse_geocode_async(latitude: float, longitude: float) -> LocationInfo:
    ...
    result = _parse_location_payload(payload)
...
def reverse_geocode(latitude: float, longitude: float) -> LocationInfo:
    """Synchronous reverse geocoding via OpenStreetMap Nominatim with rate limiting and caching."""
    cache_key = (round(latitude, 4), round(longitude, 4))
    if cache_key in _GEOCODE_CACHE:
        return _GEOCODE_CACHE[cache_key]
